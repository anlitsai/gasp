...
file_log='gasp_target_fitsheader_info_exclude_baddata_'+date1+'-'+date2+'.log'
...
#date_from=(int(date1[0:4]),int(date1[4:6]),int(date1[6:8]))
#print(date_from)

#date_end=(int(date2[0:4]),int(date2[4:6]),int(date2[6:8]))
#print(date_end)
'''
date1=date_from[0:4]+'-'+date_from[4:6]+'-'+date_from[6:8]
print(date1)
date2=date2[0:4]+'-'+date_end[4:6]+'-'+date_end[6:8]
print(date2)
times=[date1,date2]
print(times)

days=Time(times,format='iso',out_subfmt='date')
print(days)
n_days=days[1]-days[0]
#n_days=int((days[1]-days[0]).value)
print(n_days)
'''


date_from = datetime(int(date1[0:4]),int(date1[4:6]),int(date1[6:8]))
date_end = datetime(int(date2[0:4]),int(date2[4:6]),int(date2[6:8]))

info_date=('... will generate fits list from '+date1+' to '+date2+' ...')
print(info_date)
f_log.write(info_date+'\n')



n_days = date_end - date_from
print(n_days)

list_folders=[]
for i in range(n_days.days + 1):
    folder='slt'+(date_from + timedelta(days=i)).strftime("%Y%m%d")+'_calib_sci'
#    print(folder)
    list_folders.append(folder)
#    print(( date_end + timedelta(days=i)).strftime("%Y%m%d"))

#print(list_folders)
#f_log.write(list_folders+'\n')
...
start_dt = date(int(year_from), int(month_from), int(day_from))
end_dt = date(int(year_end), int(month_end), int(day_end))
...
'''
#20190810-20191031
cmd_search_folder1='find ./|grep calib | grep slt201908[1-3][0-9]| cut -d / -f3 | sort |uniq'
f_log.write(cmd_search_folder1+'\n')
list_folder1=os.popen(cmd_search_folder1,"r").read().splitlines()

cmd_search_folder2='find ./|grep calib | grep slt201909| cut -d / -f3 | sort |uniq'
f_log.write(cmd_search_folder2+'\n')
list_folder2=os.popen(cmd_search_folder2,"r").read().splitlines()

cmd_search_folder3='find ./|grep calib | grep slt201910| cut -d / -f3 | sort |uniq'
f_log.write(cmd_search_folder3+'\n')
list_folder3=os.popen(cmd_search_folder3,"r").read().splitlines()

list_folders=list_folder1+list_folder2+list_folder3
print(list_folders)
'''
...
for i in list_folders:
    cmd_search_file_sci="find ./ |grep "+i+" |grep 'fits\|fts' |sort "
#    print(cmd_search_file_sci)
#    f_log.write(cmd_search_file_sci+'\n')
    file_sci=os.popen(cmd_search_file_sci,"r").read().splitlines()
    if file_sci:
        list_file_sci.append(file_sci)

list_file_sci=[j for sub in list_file_sci for j in sub]
print(list_file_sci)
'''


#20190810-20191031
cmd_search_file_sci1="find ./201908[1-3][0-9] |grep 'fits\|fts'|grep calib_sci |sort "
print(cmd_search_file_sci1)
#f_log.write(cmd_search_file_sci1+'\n')
list_file_sci1=os.popen(cmd_search_file_sci1,"r").read().splitlines()
print(list_file_sci1)

cmd_search_file_sci2="find ./201909 |grep 'fits\|fts'|grep calib_sci |sort "
print(cmd_search_file_sci2)
#f_log.write(cmd_search_file_sci2+'\n')
list_file_sci2=os.popen(cmd_search_file_sci2,"r").read().splitlines()
print(list_file_sci2)

cmd_search_file_sci3="find ./201910 |grep 'fits\|fts'|grep calib_sci |sort "
print(cmd_search_file_sci3)
f_log.write(cmd_search_file_sci3+'\n')
list_file_sci3=os.popen(cmd_search_file_sci3,"r").read().splitlines()
print(list_file_sci3)

list_file_sci=list_file_sci1+list_file_sci2+list_file_sci3
